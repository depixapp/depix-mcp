...
import type { StatusReadResponse } from "../../../src/wallet-engine/api/client.js";
import type { WaitOptions } from "../../../src/wallet-engine/flows/status.js";
import type {
  PayLightningResult,
  ReceiveLightningResult,
  ToStablecoinResult,
} from "../../../src/wallet-engine/convert/boltz/convert.js";
import type { StablecoinParams } from "../../../src/wallet-engine/convert/boltz/stablecoin.js";
import type { ConvertParams, ConvertResult, RouteQuote } from "../../../src/wallet-engine/convert/intent.js";
import type { ConvertIntent, Route } from "../../../src/wallet-engine/convert/routes.js";
import type { SideSwapQuote, SwapExecuteResult, SwapQuoteParams } from "../../../src/wallet-engine/convert/sideswap.js";
import type { SideShiftSendResult } from "../../../src/wallet-engine/convert/sideshift.js";
import type {
  BuyGiftcardParams,
  BuyGiftcardResult,
  GiftcardCatalog,
  ...
  ListGiftcardsParams,
  ListProductsParams
} from "../../../src/wallet-engine/giftcards/namespace.js";
import type { OrderPhase } from "../../../src/wallet-engine/giftcards/cryptorefills.js";
import type { StoredGiftcardOrder } from "../../../src/wallet-engine/giftcards/store.js";
...
/**
 * MOUNT the wallet tools on a HOST-owned server (registerWalletTools — the
 * unified-bin path, §1.5) and connect a client to it. `hostTools` registers
 * stand-ins for the gateway's own tools so the mixed catalog can be asserted.
 */
export async function connectMountedWallet(
  context: RegisterWalletToolsContext = {},
  opts: { hostTools?: string[] } = {},
): Promise<{ client: Client; server: McpServer; registration: WalletToolsRegistration }> {
  const server = new McpServer({ name: "test-gateway", title: "Test Gateway", version: "1.0.0" });
  for (const name of opts.hostTools ?? []) {
    server.registerTool(name, { description: `host tool ${name}` }, () => ({
      content: [{ type: "text" as const, text: "{}" }],
    }));
  }
  const registration = registerWalletTools(server, context);
  const client = new Client({ name: "test-host", version: "1.0.0" });
  const [a, b] = InMemoryTransport.createLinkedPair();
  await Promise.all([server.connect(a), client.connect(b)]);
  return { client, server, registration };
}
