...
export class FakeSideSwapClient implements SideSwapClient {
  ...
  readonly pegQuoteCalls: Array<{ pegIn: boolean; sendAmountSats?: number | null }> = [];
  ...
  async pegQuote(args: { pegIn: boolean; sendAmountSats?: number | null }): Promise<PegQuoteResult | null> {
    this.pegQuoteCalls.push(args);
    if (this.script.pegQuote) return this.script.pegQuote(args);
    // Default: no live quote — callers fall back to serverStatus, then the floor.
    return null;
  }
  ...
  async serverStatus(args?: { forceRefresh?: boolean }): Promise<Record<string, unknown>> {
    if (this.script.serverStatus) return this.script.serverStatus(args);
    return {};
  }
}
