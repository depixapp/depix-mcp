...
describe("pegOut — guardrail on L-BTC value + the FINAL BTC recv_addr (§4.3/§5.2)", () => {
  ...
  it("passes the guardrail (kind + btcAddress destination) then reaches the L-BTC build", async () => {
    ...
    // Above the conservative peg-out floor (100_000) so the P6 minimum gate
    // clears and the flow reaches the build; the minimum gate itself is covered
    // in sideswap-pegout-min.test.ts.
    await expect(peg.pegOut({ recvAddr: "bc1qdestination", amountSats: 200_000n })).rejects.toThrow("BUILD_REACHED");
