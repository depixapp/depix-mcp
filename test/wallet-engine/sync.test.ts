...
} from "../../src/wallet-engine/sync/sync.js";
import { SCAN_HINT_MAX, UpdateStore } from "../../src/wallet-engine/store/update-store.js";
import { buildWollet, descriptorFromMnemonic, generateMnemonic } from "../../src/wallet-engine/engine/lwk.js";
...
describe("provider chain (spec §2.6)", () => {
  ...
  it("runs the default proxy providers at concurrency 8 (frontend parity, 2026-07-18)", () => {
    for (const provider of DEFAULT_ESPLORA_PROVIDERS) {
      expect(providerConcurrency(provider)).toBe(8);
    }
    // Fallback for a proxy provider supplied WITHOUT an explicit field stays 4
    // (mirrors the frontend's own fallback); anything else stays at 1.
    expect(providerConcurrency({ name: "depix-proxy-x", url: "u", waterfalls: false })).toBe(4);
    expect(providerConcurrency({ name: "some-vanilla", url: "u", waterfalls: false })).toBe(1);
  });
