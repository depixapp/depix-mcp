...
const rootUrl = new URL("../../", import.meta.url); // package root (test/wallet-engine/ is two levels down)
...
describe("viem never enters the Lightning-send chunk (stays dynamic-only, §5.3)", () => {
  ...
  for (const mod of [
    ...
    "src/wallet-engine/convert/boltz/keys.ts"
  ]) {
  ...
  it("stablecoin.ts imports viem DYNAMICALLY only (no top-level static import)", () => {
    const src = read("src/wallet-engine/convert/boltz/stablecoin.ts");
