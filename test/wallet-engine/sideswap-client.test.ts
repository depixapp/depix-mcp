...
} from "../../src/wallet-engine/convert/sideswap-client.js";
...
describe("SideSwap client — quote stream (§5.1)", () => {
  it("parses a Success quote (Sell/Base): base=send, quote=recv, fees + ttl", async () => {
    ...
    expect(q.feeAssetId).toBe(null); // "FEEASSET" is neither "Base" nor "Quote"
    expect(q.ttlMs).toBe(25_000);
    client.disconnect();
  });

  it("resolves the 'Base'/'Quote' fee label to a concrete asset id against the market", async () => {
    // Market base=DEPIX, quote=LBTC. The label names the SIDE of the pair; only
    // the client can map it to an id, and downstream compares it to the recv asset.
    for (const [label, expected] of [["Base", DEPIX], ["Quote", LBTC]] as const) {
      const { WebSocketImpl, instances } = makeFakeWebSocket((msg, ws) => {
        const p = (msg.params ?? {}) as Record<string, unknown>;
        if (p.start_quotes) ws.respond(msg.id, { start_quotes: { quote_sub_id: 7, fee_asset: label } });
        else ws.respond(msg.id, {});
      });
      const client = createSideSwapClient({ WebSocketImpl, presetMarkets: PRESET_MARKETS });
      await client.connect();
      ...
      await flush();
      instances[0]!.notify("market", {
        quote: { quote_sub_id: 7, status: { Success: { quote_id: "Q", base_amount: 1000, quote_amount: 5 } } }
      });
      expect(quotes[0]!.feeAsset).toBe(label);
      expect(quotes[0]!.feeAssetId).toBe(expected);
      client.disconnect();
    }
  });
