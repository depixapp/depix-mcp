...
describe("wallet.convert.sideswap is wired to the wallet internals (§5)", () => {
  ...
  it("pegOut passes the allowlist for a listed recv_addr and reaches the L-BTC build (INSUFFICIENT_FUNDS)", async () => {
    // A live server minimum (5_000) below this small amount so the P6 minimum
    // gate clears — at this L-BTC price a floor-sized amount would exceed the
    // R$100/tx cap. The gate itself is covered in sideswap-pegout-min.test.ts.
    const client = new FakeSideSwapClient({ serverStatus: async () => ({ min_peg_out_amount: 5_000 }) });
    wallet = await open({
      dataDir,
      passphrase: PASSPHRASE,
      mnemonic: KNOWN_MNEMONIC,
    ...
    // Listed → allowlist passes → min gate clears → peg RPC → build on an empty wallet fails.
    await expect(
      wallet.convert.sideswap.pegOut({ recvAddr: BTC_DEST, amountSats: 10_000n })
