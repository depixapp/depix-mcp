...
import { Address, AssetId, TxBuilder, mainnetNetwork } from "../../src/wallet-engine/engine/lwk.js";
...
import {
  ...
  type PsetLike
} from "../../src/wallet-engine/flows/withdraw.js";
...
// A saque hands DePix to the provider against a Pix that may never land.
// `refundAddress` is where it comes back when that happens — this wallet's own
// receive address, so a refused payout returns to the wallet that funded it
// instead of waiting on a support ticket.
describe("withdraw() sends its own receive address as refundAddress", () => {
  const OK = {
    json: {
      response: {
        withdrawalId: "w1",
  ...
  async function withdrawAndCaptureBody(response: MockResponseSpec = OK) {
    ...
    // The empty wallet has no UTXOs, so the build fails after the POST. The
    // request body is already on the wire by then, which is what we read.
    await wallet.withdraw({ ...BASE_PARAMS, amountCents: 500 }).catch(() => {});
    const post = calls.find((c) => c.url.includes("/api/withdraw"));
    return { body: JSON.parse(post?.body ?? "{}"), wallet };
  }

  it("is a real address of this wallet, not the provider's", async () => {
    const { body, wallet: w } = await withdrawAndCaptureBody();
    expect(body.refundAddress).not.toBe(EULEN_LQ1);
    // Ownership, proven by derivation: this is index 0 of THIS wallet's own
    // descriptor. `{ index }` derives without burning the counter, so reading
    // it here does not disturb what the next caller gets.
    expect(body.refundAddress).toBe(await w.getReceiveAddress({ index: 0 }));
    expect(new Address(body.refundAddress).isBlinded()).toBe(true);
  });

  // §3.1 hands out a FRESH index per call, so the refund lands on an address
  // no other flow has published — losing the privacy property would be silent.
  it("burns its own index rather than sharing one with the next deposit", async () => {
    const { body, wallet: w } = await withdrawAndCaptureBody();
    expect(await w.getReceiveAddress()).not.toBe(body.refundAddress);
  });

  it("is persisted with the request, so a crash-resumed re-POST replays the same body", async () => {
    // The Idempotency-Key is minted for one exact body. If the resume derived a
    // fresh address the replay would be a 422 body mismatch, not a retry — and
    // the withdrawal it was resuming would be orphaned at the provider. A
    // transient 409 is the case that KEEPS the record for exactly that replay.
    const { body } = await withdrawAndCaptureBody({
      status: 409,
      json: { error: { code: "withdrawal_conflict", message: "conflict" } }
    });
    const store = new PendingWithdrawals({ dataDir, passphrase: PASSPHRASE, saltB64: await saltOf(dataDir) });
    const { records } = await store.readAll();
    expect(body.refundAddress).toBeTruthy();
    expect(records).toHaveLength(1);
    expect(records[0]?.request.refundAddress).toBe(body.refundAddress);
  });
});
