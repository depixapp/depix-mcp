...
//
// The `floor-smoke` CI job runs this against a prebuilt `dist/` on the engines
// floor (Node 22.4) to prove the shipped artifact actually works there — the
// matrix's 22.x is whatever `latest-22` resolves to, which is not the floor:
...
//      (decrypt) returns the SAME words (§2.4, exercised through the engine entry).
//
// It imports the built engine entry (dist/wallet-engine/index.js) plus one
// internal built module for the engine init check — this is a repo-local script,
// so relative imports bypass the package `exports` map by design.
...
const distIndex = resolve(here, "../dist/wallet-engine/index.js");
const distLwk = resolve(here, "../dist/wallet-engine/engine/lwk.js");

// Fixed vectors — identical to test/wallet-engine/engine.test.ts (§2.1 goldens).
