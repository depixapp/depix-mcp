...
export {
  ...
  type WalletDiagnosticsPending,
  type WalletDiagnosticsSync,
  type WalletSyncCallOptions,
...
export {
  ...
  BOLTZ_API_BASE,
  BOLTZ_WS_URL,
  // Swap-provider fallback (§5.3): Boltz first, then whoever else is answering.
  SWAP_PROVIDERS,
  STABLECOIN_PROVIDER,
...
// The interactive backup ritual (§2.9). Exported so a HOST bin (the unified
// `@depixapp/mcp` init, §1.5) can drive the same ceremony instead of writing a
// second one — runWalletInit below is the wrapper that does exactly that.
export { runBackupRitual, type RitualIo, type RitualOptions } from "./backup-ritual.js";
...
export {
  ...
  type CryptorefillsFaceValue,
  type CryptorefillsFetch,
  type CryptorefillsFetchResponse,
  type CryptorefillsOrder,
  type CryptorefillsPrice,
  type CryptorefillsProduct,
  type CryptorefillsProductsEntry,
  type CryptorefillsRawProduct,
...
// MOUNT the 27 tools on a server it already owns (registerWalletTools) and run
// the first-run ceremony (runWalletInit) — the two halves the unified
// `@depixapp/mcp` bin consumes (unified-MCP spec §1.5).
export {
  ...
  PASSPHRASE_PLACEHOLDER as WALLET_INIT_PASSPHRASE_PLACEHOLDER,
  ToolError,
  mapToolError,
  missingApiKeyError,
  resolveInitCommand,
