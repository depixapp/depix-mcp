...
/**
 * The first-run command named by `wallet_not_configured` (§1.5 of the unified
 * MCP spec). The unified bin overrides it with its own package name.
 */
export const DEFAULT_WALLET_INIT_COMMAND = "npx -y @depixapp/mcp init";

// The init command is OPERATOR-supplied (the bin passes its own package name),
// never model input — but it is interpolated into a `message` the host LLM
// reads, so it is fenced to the shape of a short shell one-liner first. Anything
// else falls back to the default: a message stays a function of the code plus
// closed/guarded values only (the trust boundary at the top of this file).
const INIT_COMMAND_RE = /^[A-Za-z0-9 @/_.:-]{1,120}$/;

/**
 * The init command to put in front of a model: the caller's when it is a plain
 * shell one-liner, the default otherwise. Shared by `wallet_not_configured` and
 * the seedless `instructions` (§1.6), so both are fenced by the same rule.
 */
export function resolveInitCommand(initCommand?: string): string {
  return initCommand !== undefined && INIT_COMMAND_RE.test(initCommand)
    ? initCommand
    : DEFAULT_WALLET_INIT_COMMAND;
}

/**
 * Typed error every `wallet_*` tool returns when this server has NO wallet
 * (spec §1(b)/§1.5): the tools stay MOUNTED (a static catalog — hosts snapshot
 * `tools/list` at connect and `list_changed` support is uneven), and the model
 * is told the ONE thing that fixes it — a human act at a terminal, never a tool
 * call, because the mnemonic must not transit model context.
 */
export function walletNotConfiguredError(initCommand?: string): ToolError {
  const command = resolveInitCommand(initCommand);
  return new ToolError(
    "No local wallet is configured on this MCP server, so no wallet_* tool can run. Creating a wallet is a " +
      "HUMAN act at a terminal, never a tool call: ask the OPERATOR to run " +
      `\`${command}\` on this machine (it creates or restores the wallet, shows the 12-word backup once, and ` +
