 ...
 * receive-only, 26-tool gateway subset — accurate there, and the default of
...
/**
 * Handshake title of the UNIFIED local deployment (`npx -y @depixapp/mcp`, 62
...
export function resolveServerVersion(env: NodeJS.ProcessEnv = process.env): string {
  return env.MCP_SERVER_VERSION?.trim() || "2.8.7";
}
