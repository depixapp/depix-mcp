...
//   unified (npx @depixapp/mcp) — 60 tools, the operator's seed, local signing.
...
/** Canonical run command for the full (60-tool) local deployment. */
...
export const LEVEL_TWO_SIGNPOST =
  "This is the HOSTED, receive-only level of the DePix App MCP. The same MCP has a second level with 34 more tools — " +
...
function catalogSentence(deployment: "hosted" | "unified"): string {
  ...
  return deployment === "hosted"
    ...
    : `${base} 60 tools total: 20 gateway + 6 support-ticket + 29 local wallet_* tools + 5 account tools.`;
}
